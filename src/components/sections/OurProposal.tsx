            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              {tiers.map((tier, i) => (
                <AnimatedSection key={tier.title} delay={i * 0.1}>
                  <div className={`rounded-2xl p-7 space-y-5 h-full flex flex-col ${
                    tier.highlight
                      ? "border border-accent-purple/40 bg-accent-purple/10"
                      : "glass-card"
                  }`}>
                    <div className="space-y-1">
                      <p className={`text-2xl font-bold ${
                        tier.highlight ? "text-gradient-peach-purple" : "text-white"
                      }`}>{tier.price}</p>
                    ...
                    {tier.billing.includes("month") && (
                      <p className="text-white/30 text-xs pt-2 border-t border-white/5">
                        $1 USD per additional user / month
                      </p>
                    )}
                  </div>
                </AnimatedSection>
